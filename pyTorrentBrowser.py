...
class UrlInput(QLineEdit):
    ...
    def _return_pressed(self):
        url = QUrl(self.text())
        #url = QUrl('http://localhost/pyTorrent/')
        browser.load(url)
...
class Manager(QNetworkAccessManager):
    ...
    def _finished(self, reply):
        ...
        if str(url).find('?') <> -1:
            comando = (str(url)[str(url).find('?')+1:len(str(url))])
            PyTorrentFnc.Log(comando)
            if comando == 'extrai_arquivos':
                os.system('ExtraiLegendas.py')
            if comando == 'pyTorrent':
                os.system('pyTorrent.py')
            if comando == 'acerta_arquivos':
                os.system('AcertaArquivos.py')
            if comando == 'reload':
                self.table.update([url, str(status), content_type])
